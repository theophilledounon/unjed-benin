...
resDept.addEventListener('change', () => {
  ...
  resetSelect(resArr,  '-- Choisir l\'arrondissement --');
  if (resDept.value) fillSelect(resComm, Object.keys(GEO[resDept.value]).sort(), '-- Choisir la commune --');
});
resComm.addEventListener('change', () => {
  resetSelect(resArr, '-- Choisir l\'arrondissement --');
  if (resDept.value && resComm.value) fillSelect(resArr, GEO[resDept.value][resComm.value], '-- Choisir l\'arrondissement --');
});

// ── DÉPARTEMENT À COORDONNER ──────────────────
...
document.getElementById('candidature-form').addEventListener('submit', async function(e) {
  ...
  if (!resArr.value) { showToast('⚠️ Choisissez votre arrondissement de résidence.'); return; }
